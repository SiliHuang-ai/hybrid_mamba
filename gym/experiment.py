...
from src.evaluation.evaluate_episodes import evaluate_episode_rtg2, evaluate_episode_rtg3
from src.models.decision_transformer import HDecisionTransformer
from src.models.primitive_policy import PrimitivePolicy
from src.models.decoder_action import DecoderAction
from src.models.mlp_bc import MLPBCModel
from src.training.act_trainer import ActTrainer
...
def experiment(
    ...
    cross_episodes = K * horizon / max_ep_len
    def get_batch(batch_size=256, max_len=K):
        ...
        h_s, h_g, h_rtg, h_r, h_difference, h_timesteps, h_mask, h_loss_mask, h_d, s, a, r, d, rtg, timesteps, mask = [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []

        min_rest_batch,max_rest_batch = np.arange(batch_size),np.arange(batch_size)
        i=None

        while len(min_rest_batch)>0:
            h_s_tmp, h_g_tmp, h_rtg_tmp, h_timesteps_tmp, h_d_tmp, h_r_tmp = [],[],[],[],[],[]
            idx=0
            jump = max(1, len(min_rest_batch) // cross_episodes)

            cur_s, cur_accumlated_r = [], []
            while len(h_s_tmp)<max_len and idx<len(min_rest_batch):

                i = min_rest_batch[idx]
                traj = trajectories[int(low_sorted_inds[low_batch_inds[i]])]

                last_s, last_accmulated_r = copy.deepcopy(cur_s), copy.deepcopy(cur_accumlated_r)
                cur_s, cur_accumlated_r = [], []
                accumlated_r_tmp, ri_tmp = 0,0
                for si in range(0,len(traj['observations']),horizon):
                    h_s_tmp.append(traj['observations'][si:si + 1].reshape(1, -1, state_dim))

                    accumlated_r_tmp += traj['rewards'][si:si + horizon].sum()
                    cur_s.append(traj['observations'][si])
                    cur_accumlated_r.append(accumlated_r_tmp)

                    s_goal = None
                    if last_accmulated_r is not None:
                        for ri in range(ri_tmp, len(last_accmulated_r)):
                            if last_accmulated_r[ri] > accumlated_r_tmp:
                                tmp = min(ri + 1, len(last_accmulated_r) - 1)
                                s_goal = last_s[tmp]
                                ri_tmp = ri
                                break
                    if s_goal is None:
                        s_goal = desired_states(traj['observations'][si:], traj['rewards'][si:])

                    h_g_tmp.append(s_goal.reshape(1, -1, state_dim))
                    ...
                    # h_rtg_tmp.append(tmp[:,0,:].reshape(1,-1,1))
                    h_r_tmp.append(traj['rewards'][si:si + horizon].sum().reshape(1, -1, 1))
                    ...
                    h_d_tmp.append(d[-1][0, -1].reshape(1, -1))
            ...
            h_g[-1] = np.concatenate([np.zeros((1, max_len - tlen, state_dim)), h_g[-1]], axis=1)
            h_g[-1] = (h_g[-1] - state_mean) / state_std
            h_rtg[-1] = np.concatenate([np.zeros((1, max_len - tlen, 1)), h_rtg[-1]], axis=1) / scale
            h_r[-1] = np.concatenate([np.zeros((1, max_len - tlen, 1)), h_r[-1]], axis=1)
            h_d[-1] = np.concatenate([np.ones((1, max_len - tlen)) * 2, h_d[-1]], axis=1)
        ...
        h_g =torch.from_numpy(np.concatenate(h_g, axis=0)).to(dtype=torch.float32, device=device)
        h_rtg = torch.from_numpy(np.concatenate(h_rtg, axis=0)).to(dtype=torch.float32, device=device)
        h_r = torch.from_numpy(np.concatenate(h_r, axis=0)).to(dtype=torch.float32, device=device)
        h_d = torch.from_numpy(np.concatenate(h_d, axis=0)).to(dtype=torch.long, device=device)
        ...
        return s, a, r, d, rtg, timesteps, mask, h_s, h_g, h_rtg, h_r, h_timesteps, h_mask, h_loss_mask, h_d
    ...
    config = MambaDMConfig(d_model=variant['embed_dim'], n_layers=variant['mamba_block'])

    h_model = DecisionMamba(
        dm_config=config,
        state_dim=state_dim,
        z_dim=z_dim,
    ...
    optimizer = torch.optim.AdamW(
        itertools.chain(h_model.parameters(),Plan_to_go.parameters()),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--K', type=int, default=200)#50
    parser.add_argument('--pct_traj', type=float, default=0.0)
    parser.add_argument('--batch_size', type=int, default=64)#64
    parser.add_argument('--model_type', type=str, default='dt')
    parser.add_argument('--embed_dim', type=int, default=128)
    parser.add_argument('--z_dim', type=int, default=8)
    parser.add_argument('--n_layer', type=int, default=3)
    parser.add_argument('--mamba_block', type=int, default=2)
    ...
    parser.add_argument('--num_eval_episodes', type=int, default=5)#100
